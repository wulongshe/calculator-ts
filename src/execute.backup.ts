...
export type Diff<A extends INumber, B extends INumber> =
...
export type Sub<A extends INumber, B extends INumber> = A[0] extends B[0] ? Diff<A, [Not<B[0]>, B[1]]> : And<A, [Not<B[0]>, B[1]]>;

export type MulHelper<A extends INumber, B extends INumber, R extends INumber = INumber<true, []>> =
...
export type DivModHelper<A extends INumber, B extends INumber, R extends INumber = INumber<true, []>> =
...
export type StringToINumber<T extends CharNumber> =
...
export type INumberToNumber<T extends INumber> = `${T[0] extends true ? '' : '-'}${T[1]['length']}` extends `${infer N extends number}` ? N : never;
