...
export type ParseHelper<T extends ReadonlyString, R extends Expression = []> =
  ...
  : T extends `${infer N extends StringNumber} ${infer S extends ReadonlyString}` ? ParseHelper<S, [...R, `${N}`]>
  : T extends `${infer N extends StringNumber})${infer S extends ReadonlyString}` ? ParseHelper<`)${S}`, [...R, `${N}`]>
